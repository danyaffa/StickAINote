...
export default function NoteBoard() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      setNote({
        text: "", title: "Pro Note", 
        x: 50, y: 100, width: 700, height: 500,
  ...
  useEffect(() => {
    ...
    // Use 'any' here to prevent conflict between React.MouseEvent and Native MouseEvent
    const onMove = (e: any) => {
      if (!note) return;
      if (dragging.active) {
        setNote(prev => prev ? ({ ...prev, x: e.clientX - dragging.offsetX, y: e.clientY - dragging.offsetY }) : null);
      }
      if (resizing.active) {
        setNote(prev => prev ? ({
          ...prev,
  ...
  async function handleAiDraw() {
    ...
    } catch (e: any) { alert("Draw Failed: " + (e.message || "Unknown error")); } finally { setAiBusy(false); }
  }
  ...
  return (
    // FLOATING CONTAINER - Absolute Position for Dragging
    <div style={{
      position: "absolute",
      ...
      <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 8 }}>
         <div style={{ display: "flex", gap: 8, alignItems: "center", paddingBottom: 8, borderBottom: "1px solid rgba(0,0,0,0.1)", flexWrap: "wrap" }}>
             <button onClick={() => setMode("text")} style={{ fontWeight: mode === "text" ? "bold" : "normal" }} title="Switch to Text Mode">📝 Text</button>
             <button onClick={() => runAi("structure")} style={{ background: "linear-gradient(to right, #8b5cf6, #ec4899)", color: "white", border: "none", borderRadius: 4, padding: "4px 10px", fontWeight: "bold", cursor: "pointer" }} title="Deep Polish: Reorganize structure">✨ Deep Polish</button>
             <button onClick={() => runAi("fix")} title="Fix Grammar">Fix</button>
             <button onClick={() => runAi("summarise")} title="Summarise Text">Summarise</button>
             <select value={targetLanguage} onChange={(e) => setTargetLanguage(e.target.value)} style={{ borderRadius: 4, border: "1px solid #ccc", padding: "2px" }} title="Select Translation Language">
             ...
             <button onClick={() => runAi("translate")} title="Translate Text">Translate</button>
         </div>
         <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
             ...
             <button onClick={handleAiDraw} style={{ background: "#e0f2fe", border: "1px solid #38bdf8", borderRadius: 4, padding: "2px 8px", cursor: "pointer" }} title="Generate Image">
                 {note.lastImagePrompt ? "🎨 Refine" : "🖼️ AI Draw"}
             </button>
             <button onClick={handleHandwriting} title="Convert Handwriting">✍️ Text</button>
             <button onClick={handleDetect} title="Object Detection">👁 Detect</button>
             <button onClick={handleClean} title="Clean Layout">🧹 Clean</button>
             <button onClick={handleClear} style={{color: "red", cursor:"pointer"}} title="Clear Canvas">Clear</button>
