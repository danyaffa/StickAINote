    ...
    <div style={{
      ...
      <div style={{ flex: 1, position: "relative", background: "rgba(255,255,255,0.4)", borderRadius: 12, overflow: "hidden", minHeight: 0 }}>
         ...
         {mode === "draw" && (
             <div style={{ position: "absolute", top: 10, right: 10, display: "flex", gap: 4, background: "white", padding: 4, borderRadius: 6, zIndex: 20, border:"1px solid #ddd" }}>
                 ...
                 <button onClick={handleZoomReset} style={{cursor:"pointer"}} title="Reset Zoom">R</button>
             </div>
         )}
      ...
      <div style={{ marginTop: 12, display: "flex", flexDirection: "column", gap: 8 }}>
         ...
         <div style={{ display: "flex", gap: 8, alignItems: "center", paddingBottom: 8, borderBottom: "1px solid rgba(0,0,0,0.1)", flexWrap: "wrap" }}>
             <button onClick={() => setMode("text")} style={{ fontWeight: mode === "text" ? "bold" : "normal" }} title="Switch to Text Editor">📝 Text</button>
             <button onClick={() => runAi("structure")} style={{ background: "linear-gradient(to right, #8b5cf6, #ec4899)", color: "white", border: "none", borderRadius: 4, padding: "4px 10px", fontWeight: "bold", cursor: "pointer" }} title="Deep Polish: Reorganize and structure your note">✨ Deep Polish</button>
             <button onClick={() => runAi("fix")} title="Fix spelling and grammar">Fix</button>
             <button onClick={() => runAi("summarise")} title="Summarise text into bullet points">Summarise</button>
             
             <select value={targetLanguage} onChange={(e) => setTargetLanguage(e.target.value)} style={{ borderRadius: 4, border: "1px solid #ccc", padding: "2px" }} title="Select language for translation">
             ...
             <button onClick={() => runAi("translate")} title="Translate text to selected language">Translate</button>
         </div>

         {/* ROW 2: DRAWING & UTILITIES */}
         <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
             <button onClick={() => setMode("draw")} style={{ fontWeight: mode === "draw" ? "bold" : "normal" }} title="Switch to Drawing Canvas">✏️ Draw</button>
             <button onClick={handleUndo} title="Undo last stroke" style={{cursor:"pointer"}}>↩️</button>
             <button onClick={handleRedo} title="Redo last stroke" style={{cursor:"pointer"}}>↪️</button>

             <button onClick={handleAiDraw} style={{ background: "#e0f2fe", border: "1px solid #38bdf8", borderRadius: 4, padding: "2px 8px", cursor: "pointer" }} title="Generate or refine an image with AI">
                 {note.lastImagePrompt ? "🎨 Refine" : "🖼️ AI Draw"}
             </button>
             
             <button onClick={handleHandwriting} title="Convert handwriting to typed text">✍️ Text</button>
             <button onClick={handleDetect} title="Detect objects in your drawing">👁 Detect</button>
             <button onClick={handleClean} title="Clean up and straighten lines">🧹 Clean</button>
             <button onClick={handleClear} style={{color: "red", cursor:"pointer"}} title="Clear the entire canvas">Clear</button>
             ...
             {/* Export/Import */}
             <button onClick={handleExport} title="Save Note to File" style={{cursor:"pointer"}}>💾</button>
             <button onClick={triggerImport} title="Load Note from File" style={{cursor:"pointer"}}>📂</button>
         </div>
      </div>
    </div>
  );
